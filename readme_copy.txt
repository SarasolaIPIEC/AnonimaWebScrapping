...
## Reporte v2
- Descripción: reporte HTML accesible con dos vistas (Aporte% y Unitario), ordenamiento con `aria-sort`, filtros y paginación, formato numérico es-AR, panel de Metodología y descargas (CSV serie/breakdown, diccionario, log de sustituciones).
- Compatibilidad: la CLI y rutas de salida se mantienen. `src/reporting/render.py` puede correrse de forma independiente:
  - `python -m src.reporting.render --period YYYY-MM`
- Captura (demo):

![Reporte v2 — demo](docs/report_v2_example.svg)
